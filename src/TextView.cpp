...
void TextView::callbackDraw() {
    ...
    cairo_rectangle(cr, GUTTER_SIZE + 5.0, (cy-ln) * (font_size + (font_size/2.0)), w(), ((1.7*font_size)));
    cairo_fill(cr);
	...
	while (iter < end) {
	    cairo_rectangle(cr,
			    GUTTER_SIZE + 5,
			    (iter-ln) * (font_size + (font_size/2.0)),
			    w() - GUTTER_SIZE,
			    (1.7*font_size));
	    iter++;
	}
	if ( sy < cy ) {
	    cairo_rectangle(cr,
			    ...
			    (sy-ln) * (font_size + (font_size/2.0)),
			    w() - GUTTER_SIZE,
			    (1.7*font_size));
	    cairo_rectangle(cr,
			    GUTTER_SIZE + 5,
			    (cy-ln) * (font_size + (font_size/2.0)),
			    cx*ex.x_advance,
			    (1.7*font_size));
	}
	if ( sy > cy ) {
